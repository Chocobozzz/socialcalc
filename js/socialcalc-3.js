...
SocialCalc.format_text_for_display = function(rawvalue, valuetype, valueformat, sheetobj, linkstyle, nontextvalueformat) {
   ...
   if (valueformat=="text-html") { // HTML - output as it as is
      displayvalue = window.sanitizeHTML(displayvalue);
   }
...
SocialCalc.DetermineValueType = function(rawvalue) {
   ...
   if (value.length==0) {
      ...
      var navigatorLang = typeof navigator !== "undefined" ? navigator.language : ''
      value = ((navigatorLang).indexOf("fr") === 0)  ? (SocialCalc.FormatNumber.convert_date_gregorian_to_julian(year, matches[2]-0, matches[1]-0)-2415019) : (SocialCalc.FormatNumber.convert_date_gregorian_to_julian(year, matches[1]-0, matches[2]-0)-2415019);
      type = "nd";
      }
